...
요청이 많은 서버에서 여러 트랜젹션이 동시에 같은 데이터를 변경 할 경우에
데이터 유실(두번 업데이트) 이나 장애로 이어질 수 있습니다. 

구현된 뿌리기는 사용자가 뿌리기를 하게 되면 사용자 수 만큼 미리 금액을 램덤으로 설정 하여 저장 한 뒤, 해당 대화방의 사용자들은 할당되지
않은 금액을 획득하는 방식 입니다. 이에 획득(받기 API)에 대한 경합이 발생하여, 데이터 일관성이 깨질수 있습니다.

이에 대한 해결 책으로 JPA 에서 제공하는 PESSIMISTIC(비관적 잠금) 적용 하여 동시성을 확보 합니다.
* PESSIMISTIC_WRITE : Exclusive Lock을 획득하고 데이터를 읽거나, 업데이트하거나, 삭제하는 것을 방지합니다.
